...
基于 Next.js 构建的多用户 Cloudflare DNS 记录管理平台，支持多账号隔离、权限控制和完整的 DNS 记录管理。
...
- 用户与 CF 账号绑定（多对多），VIEWER 仅可访问已绑定账号
...
Token 需要至少 **区域 → DNS → 编辑** 和 **区域 → 区域 → 读取** 权限。
