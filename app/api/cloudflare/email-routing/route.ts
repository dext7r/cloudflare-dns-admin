...
import { listEmailRoutingRules, createEmailRoutingRule } from "@/lib/cloudflare"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await listEmailRoutingRules(zoneId, token)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createEmailRoutingRule(zoneId, body, token)
    return NextResponse.json({ success: true, result })
  } catch (e) {
