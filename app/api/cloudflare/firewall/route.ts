...
import { listFirewallRules, createFirewallRule } from "@/lib/cloudflare"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await listFirewallRules(zoneId, token)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createFirewallRule(zoneId, body, token)
    return NextResponse.json({ success: true, result })
  } catch (e) {
